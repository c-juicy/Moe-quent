...
class DeepSeekV2MoEPatch(BasePatch):
    ...
    def patch_linearlayers(
        ...
        for i in tqdm(range(len(layers)), disable=not verbose):
            ...
            layers[i].self_attn.kv_a_proj_with_mqa = patch_fct(
                layers[i].self_attn.kv_a_proj_with_mqa, patch_params["self_attn.kv_a_proj_with_mqa"]
            )
            layers[i].self_attn.kv_b_proj = patch_fct(
                layers[i].self_attn.kv_b_proj, patch_params["self_attn.kv_b_proj"]
            )
...
class DeepSeekV2MoEMiLo(DeepSeekV2MoEPatch, BaseCKYHFModel):
    pass