...
from Cky.quentizers.quentizer import CkyLinear
from Cky.utils.utils import cleanup
...
_QUANT_LAYERS = [nn.Linear, CkyLinear] +  _MiLo_BACKEND_CLASSES
...
class BaseCKYModel:
    ...
    def save_weights(cls, weights: dict, save_dir: str) -> None:
        ...
        logger.info("> Compressed Weight saved")
    ...
    def compress_model(
        ...
        def _patch_linear(linear_layer, compress_config):
            if type(linear_layer) is CkyLinear:
                return linear_layer

            current_device = device_map[linear_layer.name]

            if compress_config is not None:
                logger.info('[patch linear] compress_config is not None')
                out_module = CkyLinear(
                    linear_layer,
                    compress_config,
